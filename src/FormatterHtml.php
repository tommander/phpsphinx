...
class FormatterHtml implements FormatterInterface
{
     ...
     * @return string
     */
    public static function typeToHtml(string $type, string $name, string $content): string
    {
    ...
    {
        $file_template_path = Helper::makePath(__DIR__, '..', 'templates', 'html', 'file.html');
        $file_template = '';
        ...
        foreach ($hierarchy as $hier_item) {
            ...
            if ('' !== $hier_docblock || 'namespace' === $hier_type) {
                ...
                $hierarchy_content .= self::typeToHtml($hier_type, $hier_name, $hier_rst,) . PHP_EOL;
            }
        }
    ...
    {
        $file_template_path = Helper::makePath(__DIR__, '..', 'templates', 'html', 'generated.html');
        $file_template = '';
        if (file_exists($file_template_path) === true) {
            $file_template = file_get_contents($file_template_path);
        }
    ...
    public static function getEmptyIndex(string $title = ''): string
    {
        ...
        $index_template_path = Helper::makePath(__DIR__, '..', 'templates', 'html', 'index.html');
        $index_template = '';
