...
class FormatterRst implements FormatterInterface
{
     ...
     * @return string
     */
    public static function typeToRst(string $type, string $name, string $content): string
    {
        $directive = '';
     ...
     * @return string
     */
    public static function fixIndentation(string $text, int $min_indent): string
    {
        if ($min_indent <= 0) {
            return $text;
        }
    ...
    public static function format(string $title, array $hierarchy, string $commit = '', string $file_rel = ''): string  // phpcs:ignore Squiz.Commenting.FunctionComment.IncorrectTypeHint
    {
        ...
        $file_template_path = Helper::makePath(__DIR__, '..', 'templates', 'rst', 'file.rst');
        $file_template = '';
        ...
        foreach ($hierarchy as $hier_item) {
            $hier_docblock = Helper::makeString($hier_item['docblock']);
            $hier_type = Helper::makeString($hier_item['type']);
            if ('' !== $hier_docblock || 'namespace' === $hier_type) {
                $hier_name = Helper::makeString($hier_item['name']);
                $phpdoc_data = PhpDoc::getPhpdocData("<?php\n" . $hier_docblock);
                $hier_rst = self::outputStr($phpdoc_data['description'], $phpdoc_data['data']);
                $hierarchy_content .= self::fixIndentation(self::typeToRst($hier_type, $hier_name, self::fixIndentation($hier_rst, $indent + 1)), $indent) . PHP_EOL;
            }
            if (in_array($hier_type, array( 'class', 'interface', 'trait' ))) {
                $indent = 1;
            }
        }
        $file_content = self::substitute($file_template, '', self::generatedAutomatically(gmdate('c'), $commit, $file_rel), $hierarchy_content, '');
        return sprintf(
            $file_content,
            $title,
            $underline
     ...
     * @return string
     */
    public static function commitLink(string $commit): string
    {
        if ('' === trim($commit)) {
            return '';
        }
     ...
     * @return string
     */
    public static function fileLink(string $commit, string $file): string
    {
        if ('' === trim($file)) {
            return '';
        }
     ...
     * @return string
     */
    public static function generatedAutomatically(string $date, string $commit, string $file): string
    {
        $file_template_path = Helper::makePath(__DIR__, '..', 'templates', 'rst', 'generated.rst');
        $file_template = '';
        if (file_exists($file_template_path) === true) {
            $file_template = file_get_contents($file_template_path);
        }
     ...
     * @return string
     */
    public static function getEmptyIndex(string $title = ''): string
    {
        if ('' === $title) {
            $title = 'Subfolder';
        }
        ...
        $index_template_path = Helper::makePath(__DIR__, '..', 'templates', 'rst', 'index.rst');
        $index_template = '';
     ...
     * @return string
     */
    public static function addToIndex(string $what, string $subfolder_index): string
    {
     ...
     * @return string
     */
    public static function outputStr(string $description, array $data): string  // phpcs:ignore Squiz.Commenting.FunctionComment.IncorrectTypeHint
    {
